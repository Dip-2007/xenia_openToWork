...
function createBeam(width: number, height: number): Beam {
    ...
    return {
        ...
        opacity: 0.08 + Math.random() * 0.12, // Lowered for light mode
...
export function BeamsBackground({
    ...
    useEffect(() => {
        ...
        function drawBeam(ctx: CanvasRenderingContext2D, beam: Beam) {
            ...
            // Light mode hues: focus on sky blues and cyans
            gradient.addColorStop(0, `hsla(${beam.hue}, 80%, 70%, 0)`);
            gradient.addColorStop(
                0.1,
                `hsla(${beam.hue}, 80%, 70%, ${pulsingOpacity * 0.5})`
            );
            gradient.addColorStop(
                0.4,
                `hsla(${beam.hue}, 80%, 70%, ${pulsingOpacity})`
            );
            gradient.addColorStop(
                0.6,
                `hsla(${beam.hue}, 80%, 70%, ${pulsingOpacity})`
